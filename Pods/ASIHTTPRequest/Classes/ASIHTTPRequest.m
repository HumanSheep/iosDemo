...
- (void)setReadStream:(NSInputStream *)stream
{
    readStream = stream;
}
+ (void)initialize
...
{
    NSInputStream *stream = [self readStream];
    NSString *httpVersion = nil;
    if(stream != nil)
    {
        CFHTTPMessageRef message = (CFHTTPMessageRef)CFReadStreamCopyProperty((CFReadStreamRef)stream, kCFStreamPropertyHTTPResponseHeader);
        httpVersion = [NSMakeCollectable(CFHTTPMessageCopyVersion(message)) autorelease];
        CFRelease(message);
    }
